  ...
  useEffect(() => {
    async function loadListings() {
      const { data, error } = await supabase
        .from("store_listings")
        .select("id, title, category, condition, warranty_months, retail_price, image_url, description")
        .eq("published", true)
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        <section id="inicio" className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 lg:grid-cols-2 lg:py-24">
          ...
          <div className="grid min-h-[330px] grid-cols-2 gap-3 rounded-2xl bg-secondary p-3 sm:min-h-[470px]"><div className="rounded-xl bg-[linear-gradient(145deg,hsl(var(--primary)),hsl(var(--sidebar-background)))]" /><div className="mt-12 rounded-xl bg-card shadow-sm" /><div className="rounded-xl bg-card shadow-sm" /><div className="mb-12 rounded-xl bg-[linear-gradient(145deg,hsl(var(--muted)),hsl(var(--primary)))]" /></div>
        </section>

        <section id="stock" className="mx-auto max-w-7xl scroll-mt-20 px-5 py-16 sm:px-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Catálogo</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">Stock disponível</h2><p className="mt-3 text-muted-foreground">Equipamentos selecionados, com estado e garantia apresentados com clareza.</p>{loading ? <p className="mt-10 text-muted-foreground">A carregar stock...</p> : listings.length === 0 ? <div className="mt-10 rounded-xl border border-dashed border-border p-8 text-muted-foreground">Ainda não existem artigos publicados. Os produtos publicados no painel de administração aparecerão aqui.</div> : <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{listings.map((product) => <article key={product.id} className="overflow-hidden rounded-xl border border-border bg-card"><div className="aspect-[4/3] bg-secondary">{product.image_url ? <img src={product.image_url} alt={product.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm font-semibold text-muted-foreground">Vértice</div>}</div><div className="p-5"><p className="text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground">{product.category}</p><h3 className="mt-2 text-lg font-semibold">{product.title}</h3><p className="mt-1 text-sm text-muted-foreground">Estado: {product.condition} · Garantia: {product.warranty_months} meses</p>{product.description && <p className="mt-3 text-sm text-muted-foreground">{product.description}</p>}<div className="mt-5 flex items-center justify-between gap-3"><span className="text-xl font-bold">{euros(Number(product.retail_price))}</span><Button variant="outline" onClick={() => openContact(`Interesse em comprar: ${product.title}`)}>Comprar</Button></div></div></article>)}</div>}</section>
