  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:py-14">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="lg:pt-4"><p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{product.category}</p><h1 className="mt-3 text-4xl font-extrabold tracking-tight sm:text-5xl">{product.title}</h1><p className="mt-4 text-lg text-muted-foreground">Estado: {product.condition}</p><p className="mt-6 text-3xl font-extrabold">{Number(product.retail_price) > 0 ? euro(Number(product.retail_price)) : "Sob consulta"}</p><a href="/#contacto" className="mt-7 inline-flex min-h-12 items-center justify-center rounded-md bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:opacity-90">Tenho interesse neste produto</a><div className="mt-9 space-y-4 border-t border-border pt-7"><div className="flex gap-3"><ShieldCheck className="mt-0.5 shrink-0" size={21} /><div><h2 className="font-bold">Equipamento verificado</h2><p className="mt-1 text-sm text-muted-foreground">Inspecionado antes de ser colocado à venda.</p></div></div>{product.warranty_months > 0 && <div className="flex gap-3"><Check className="mt-0.5 shrink-0" size={21} /><div><h2 className="font-bold">Garantia de {product.warranty_months} meses</h2><p className="mt-1 text-sm text-muted-foreground">Cobertura indicada de forma clara.</p></div></div>}</div></div>
      </div>
